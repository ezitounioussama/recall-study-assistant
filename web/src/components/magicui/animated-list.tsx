...
/** MagicUI AnimatedList, vendored: children appear one at a time, newest on top, with a spring. */
import React, { useEffect, useMemo, useState, type ComponentPropsWithoutRef, type ReactNode } from "react";
...
export const AnimatedList = React.memo(({ children, className, delay = 1000, ...props }: AnimatedListProps) => {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => setIndex((i) => (i + 1) % childrenArray.length), delay);
